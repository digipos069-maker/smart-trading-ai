...
OANDA_SYMBOLS = {"XAUUSD", "XAU_USD", "EURUSD", "EUR_USD", "GBPUSD", "GBP_USD", "BTCUSD", "BTC_USD", "NQ", "NAS100_USD"}
OANDA_SYMBOL_MAP = {
    "XAUUSD": "XAU_USD",
    "EURUSD": "EUR_USD",
    "GBPUSD": "GBP_USD",
...
class OandaMarketDataProvider:
    ...
    def status(self) -> ProviderStatus:
        ...
        return {
            "provider": self.name,
            "status": "configured",
            "server": settings.OANDA_BASE_URL,
        }
    ...
    def validate_symbol(self, symbol: str) -> str:
        ...
        if normalized not in OANDA_SYMBOLS:
            ...
            raise ValueError(
                f"Unsupported OANDA symbol '{symbol}'. Supported symbols: {supported}"
            )
    ...
    def validate_timeframe(self, timeframe: str) -> str:
        ...
        if normalized not in OANDA_TIMEFRAMES:
            ...
            raise ValueError(
                f"Unsupported OANDA timeframe '{timeframe}'. Supported timeframes: {supported}"
            )
        return normalized
...
def _parse_oanda_time(value: str) -> datetime:
    # OANDA returns nanosecond timestamps; Python datetime supports microseconds.
    if "." in value:
